...
var palette = [
    new Color(1, 143, 39, 26),
    new Color(1, 13, 113, 125),
    new Color(1, 72, 176, 19)
];
...
var ColumnSeries = function (chart) {
    ...
    this._color = new Color(1, 255, 0, 0);
};
...
ColumnSeries.prototype._render = function (ctx) {
    ...
    f.widths = [];
    f.heights = [];
    currColor = this._color.toString();
...
for (i = 0; i < data.length; i++) {
    ...
    f.categoryStart.push(scaledX);
    f.columnCenter.push(scaledX + offset);
    ...
    for (var i = 0; i < f.widths.length; i++) {
        ...
        if (this._index == 0) {
            ctx.fillStyle = "black";
            //ctx.fillRect(f.categoryStart[i] - 2, this._chart._plotTop, 4, this._chart._plotHeight);
            //ctx.fillStyle = "green";
            ctx.fillRect(f.columnCenter[i] - 2, this._chart._plotTop, 4, this._chart._plotHeight);
        }
    }
};
